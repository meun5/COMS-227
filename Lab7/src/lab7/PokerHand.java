...
public class PokerHand {
	...
	public boolean isStraight() {
		...
		for (int z = 0; z < cards.length - 1; z++) {
			if (cards[z].getRank() != cards[z + 1].getRank() - 1) {
				return false;
			}
		}
	...
	/**
	 * Returns the index of the minimal element to the right of position
	 * 'start', where cards are ordered first by suit, and within the same suit,
	 * by rank.
	 * 
	 * @param start
	...
	private int findIndexOfMin(int start) {
		...
		for (int j = start + 1; j < cards.length; ++j) {
			...
			// if current has a lower suit than the current min, then it should
			// come
			// first
